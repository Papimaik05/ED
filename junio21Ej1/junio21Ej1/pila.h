	...
	**/
	void minimoACima() {
		...
		while (_cima != NULL) {
			...
			auxpila.apila(_cima->_elem);
			this->desapila();
			contador++;
		}

		contador -= 1;
		posicionmin = contador - posicionmin ;
		Nodo* auxcima = auxpila._cima;
		auto minimo = auxpila._cima->_elem;
		contador = 0;
		while (auxcima != NULL) {
			if (contador == posicionmin) {
				minimo = auxcima->_elem;
				auxcima = auxcima->_sig;
				auxpila.desapila();
			}
			else {
				this->apila(auxcima->_elem);
				auxcima = auxcima->_sig;
				auxpila.desapila();
			}
			contador++;
