...
public:
	...
	*/
	...
	void concatena(const Lista<T>&lista){
		int i = 0;
		while (i < lista._numElems) {
			this->pon_final(lista._prim);
			i++;
			lista.borraElem(lista, lista._prim);
		}	
		Nodo* ptr=lista
	}
	void concatena(const Lista<T>& lista) {
	...
	void concatena2(const Lista<T>& lista) {
		if (esVacia()) {
			_prim = lista_prim;
			_ult = lista_ult;
			_numElems = lista_numElems;
